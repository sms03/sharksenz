  ...
  return (
    <MainLayout>
      <div className="mx-auto max-w-5xl py-12">
        ...
        <div className="flex justify-center mb-8 gap-4">          <button
            className={`px-4 py-2 rounded border h-10 w-24 ${currency === 'USD' ? 'bg-shark-500 text-white' : 'bg-white text-shark-500 border-shark-500'}`}
            onClick={() => setCurrency('USD')}
          >
            USD
          </button>
          <button
            className={`px-4 py-2 rounded border h-10 w-24 ${currency === 'INR' ? 'bg-shark-500 text-white' : 'bg-white text-shark-500 border-shark-500'}`}
        ...
        <div className="grid gap-8 md:grid-cols-3">
          {tiers.map((tier) => (            <Card key={tier.name} className={`${tier.highlight ? "border-2 border-shark-500 shadow-lg" : ""} h-[450px]`}>
              ...
              <CardContent className="h-[300px]">
                <ul className="mb-6 space-y-2">
                  ...
                  <Button className="w-full h-10" variant={tier.highlight ? "default" : "outline"} onClick={() => handleCTAClick(tier.name)}>{tier.cta}</Button>
                ) : (
                  <Dialog open={selectedTier === tier.name} onOpenChange={closeDialog}>
                    ...
                    <DialogContent>
                      ...
                      <DialogFooter>                        <Button className="w-full h-10" onClick={() => { alert('Redirecting to payment gateway...'); closeDialog(); }}>
                          Proceed to Payment
                        </Button>
                      </DialogFooter>
                    </DialogContent>
